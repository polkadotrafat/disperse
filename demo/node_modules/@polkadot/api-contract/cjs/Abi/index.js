...
function getLatestMeta(registry, json) {
  ...
  (0, _util.assert)(converter, () => `Unable to convert ABI with version ${metadata.type} to latest`);
...
class Abi {
  ...
  #decodeMessage = (type, list, data) => {
    ...
    (0, _util.assert)(data.length === args.length, () => `Expected ${args.length} arguments to contract message '${label.toString()}', found ${data.length}`);
